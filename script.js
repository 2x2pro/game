document.addEventListener('DOMContentLoaded', function() {
    ...
    gamesSliders.forEach(slider => {
        const gamesContainer = slider.querySelector('.games-container');
        ...
        if (gamesContainer && prevButton && nextButton) {
            ...
            const visibleCards = Math.floor(gamesContainer.offsetWidth / cardWidth);
            const scrollAmount = cardWidth * visibleCards;
            let currentSlide = 0;
            const maxSlides = dots ? dots.length : Math.ceil(gamesContainer.scrollWidth / scrollAmount);

            // Initialize first dot as active if dots exist
            if (dots.length) updateDots(0);

            // Next button click
            nextButton.addEventListener('click', function() {
                const maxScroll = gamesContainer.scrollWidth - gamesContainer.offsetWidth;
                const newScrollPosition = Math.min(gamesContainer.scrollLeft + scrollAmount, maxScroll);
                gamesContainer.scrollTo({ left: newScrollPosition, behavior: 'smooth' });
                currentSlide = Math.min(currentSlide + 1, maxSlides - 1);
                if (dots.length) updateDots(currentSlide);
            });

            // Previous button click
            prevButton.addEventListener('click', function() {
                const newScrollPosition = Math.max(gamesContainer.scrollLeft - scrollAmount, 0);
                gamesContainer.scrollTo({ left: newScrollPosition, behavior: 'smooth' });
